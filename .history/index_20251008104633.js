...
// ...depois de pegar value e msg...
const to = `+${msg.from}`;
const name = value?.contacts?.[0]?.profile?.name || '';

// ===== SESSÃO com expiração de 30min =====
...
if (msg.type === 'text') {
  ...
  if (['reset','reiniciar','recomeçar','inicio','início'].includes(t)) {
    ...
    // reenvia a saudação (mesmo bloco que você usa no stage 'new')
    const saud = (CONFIG.saudacao || `Olá${name ? `, ${name}` : ''}! 👋\n\n` +
      `Tenho duas opções pra você:\nA) ${CONFIG.produtoA.titulo} — ${CONFIG.produtoA.preco}\n` +
      `B) ${CONFIG.produtoB.titulo} — ${CONFIG.produtoB.preco}\n\n` +
    ...
    try {
      ...
      console.error('[RESET] erro ao enviar saudação:', e?.response?.data || e.message);
    }
    return;
  }
}
