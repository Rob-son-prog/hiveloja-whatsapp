...
// servir a pasta /checkout
const staticCheckoutPath = require('path').join(__dirname, 'checkout');
app.use('/checkout', express.static(staticCheckoutPath));

// fallback explícito para /checkout (sem a barra)
app.get('/checkout', (req, res) => {
  res.sendFile(require('path').join(staticCheckoutPath, 'index.html'));
});
...
app.post('/upload', upload.single('file'), (req, res) => {
  try {
    ...
    // se quiser URL completa, defina APP_BASE_URL no .env (ex.: seu túnel Cloudflare)
...
async function sendGreeting(to, name) {
  ...
  // 1) texto completo da saudação (o Web SEMPRE mostra)
  await sendText({
    token: process.env.WHATSAPP_TOKEN,
    phoneNumberId: process.env.PHONE_NUMBER_ID,
    to,
    body, // "Olá, {NAME}! ..." já com A/B e preços
  });
  ...
  // 3) depois disso, tenta enviar os botões (aparecem no celular)
  try {
    await sleep(400); // pequeno intervalo ajuda na ordem de renderização
    await sendButtons({
      token: process.env.WHATSAPP_TOKEN,
      phoneNumberId: process.env.PHONE_NUMBER_ID,
      to,
      body, // mesmo texto como corpo dos botões
      buttons: [
    ...
    // tudo bem: o texto já foi enviado acima
  }
}
...
async function sendOffer(to, product, orderId) {
  ...
  // 1) texto da oferta (aparece no Web)
  await sendText({
    token: process.env.WHATSAPP_TOKEN,
    phoneNumberId: process.env.PHONE_NUMBER_ID,
    to,
  ...
  // 2) lembrete para voltar ao menu no Web/PC
  await sendText({
    token: process.env.WHATSAPP_TOKEN,
    phoneNumberId: process.env.PHONE_NUMBER_ID,
    to,
  ...
  // 3) opcional: no celular, mandar um botão "Menu"
  try {
    await sendButtons({
      token: process.env.WHATSAPP_TOKEN,
      phoneNumberId: process.env.PHONE_NUMBER_ID,
