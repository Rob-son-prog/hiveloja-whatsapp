...
let CONFIG = {
  ...
  produtoA: {
    ...
    preco: 'R$ 19,90',
    // para Mercado Pago automático via WhatsApp, troque para "mp:auto"
...
async function createMPPreferenceForProduct(productKey, orderId) {
  ...
  const preference = {
    items: [
      { title, quantity: 1, unit_price, currency_id: 'BRL' }
    ],
    binary_mode: true,
...
// SDK v2 retorna campos no próprio objeto de resposta
...
// Rota: cria preference e retorna init_point
...
// Rota: gera preference e REDIRECIONA ao init_point (ideal pro botão "Comprar")
...
// Webhook do MP (opcional, apenas loga por enquanto)
app.post('/mp/webhook', (req, res) => {
  try {
    console.log('[MP WEBHOOK]', JSON.stringify(req.body || req.query || {}, null, 2));
  } catch {}
  res.sendStatus(200);
});
// ============================================================================
...
async function sendOffer(to, product, orderId) {
  // Se checkout_url começar com "mp:", cria a preferência do MP
  let link;
  ...
  if (urlTpl.startsWith('mp:')) {
    try {
      ...
      link = buildCheckoutUrl(urlTpl, orderId); // fallback (não deve acontecer)
    }
  } else {
...
const urlTpl = String(product?.checkout_url || '').trim();
if (urlTpl.startsWith('mp:')) {
  // ... cria preference do MP e atribui link = init_point
} else {
  link = buildCheckoutUrl(urlTpl, orderId);
}
