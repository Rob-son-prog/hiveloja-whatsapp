...
function writeJsonSafe(p, v){ fs.writeFileSync(p, JSON.stringify(v, null, 2)); }

let ANALYTICS = readJsonSafe(ANALYTICS_PATH, []); // [{ts, type, productKey?, orderId?, wa_id?, amount?}]
function logEvent(ev){
  ev = { ts: Date.now(), ...ev };
  ANALYTICS.push(ev);
  writeJsonSafe(ANALYTICS_PATH, ANALYTICS);
}
