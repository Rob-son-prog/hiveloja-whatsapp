...
// === MP: receber submissões do Brick e criar o pagamento ===
app.post('/mp/process-payment', async (req, res) => {
  try {
    if (!mpClient) return res.status(400).json({ ok:false, error:'MP_ACCESS_TOKEN ausente' });

    // O Brick envia os campos corretos para cada método (cartão/pix/boleto).
    // Vamos montar com segurança e ainda incluir nossos metadados.
    const {
      transaction_amount,
      description,
      payment_method_id,
    ...
    const body = {
      ...
      payment_method_id,        // 'pix', 'credit_card', 'bolbradesco' (o Brick envia o correto)
      token,
    ...
    return res.json({
      ok: true,
      id: resp?.id || resp?.body?.id,
      status: resp?.status || resp?.body?.status,
      status_detail: resp?.status_detail || resp?.body?.status_detail
    });
  } catch (e) {
...
app.post('/mp/webhook', async (req, res) => {
  ...
  try {
    ...
    // >>> NOVO: marca o contato como "comprou" e atualiza lastSeen
    try {
    ...
    // --- analytics: registra a compra aprovada
    try {
