...
app.post('/mp/process-payment', async (req, res) => {
  try {
    ...
    if (req.body && req.body.method) {
      ...
      // BOLETO
      if (method === 'boleto') {
        // Em BR, geralmente payment_method_id = 'bolbradesco'
        const body = {
          transaction_amount: Number(amount),
          description: title,
          ...
          payer: {
            ...
            last_name: req.body?.payer?.last_name || 'MercadoPago',
            identification: req.body?.payer?.identification || { type:'CPF', number:'00000000000' }
          },
          binary_mode: true,
          metadata: { orderId, productKey }
        };
        ...
        return res.json({
          ...
          status: p?.status,
          status_detail: p?.status_detail,
          boleto: { external_resource_url: url, url }
        });
      }

      // CARTÃO -> o front já faz fallback para Checkout Pro
      if (method === 'card') {
        return res.json({ ok:true, method:'card', orderId });
      }
    }

    // === Caminho 1: BRICKS (mantido p/ compatibilidade) =============
    const {
      transaction_amount,
      description,
      payment_method_id,
