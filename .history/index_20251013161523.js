...
app.post('/mp/process-payment', async (req, res) => {
  try {
    ...
    // ✅ Use um único Payment por requisição
    ...
    if (req.body && req.body.method) {
      ...
      // ===== BOLETO (com endereço obrigatório) =====
      if (method === 'boleto') {
        ...
        const address = {
          ...
          federal_unit:  String(addrIn.federal_unit  || '').toUpperCase().trim()
        };
        ...
        if (missing.length) {
          return res.status(400).json({ ok:false, error:'Endereço do pagador incompleto para boleto', required: missing });
        }
    ...
    return res.status(400).json({ ok:false, error: e?.response?.data || e.message });
      ...
      if (method === 'boleto') {
        const boleto = req.body?.boleto || {};
        const rawName = String(boleto.name || 'Cliente Padrão').trim();
        const cpf     = String(boleto.cpf  || '').replace(/\D+/g, '');
        const email   = String(boleto.email|| 'compras@example.com').trim();
        ...
        // Mercado Pago pede first_name e last_name separados
        const parts = rawName.split(/\s+/).filter(Boolean);
        const first_name = parts.length ? parts[0] : 'Cliente';
        const last_name  = parts.length > 1 ? parts.slice(1).join(' ') : 'MP';

        const txAmount = Number(amount);
        // muitos PSPs exigem mínimo ~R$3,00 para boleto
        if (!(txAmount >= 3)) {
          return res.status(400).json({ ok:false, error:'Valor mínimo para boleto é R$ 3,00' });
        }
        ...
        const body = {
          ...
          payment_method_id: 'bolbradesco',
          external_reference: orderId,
          binary_mode: false, // boleto pode ficar pendente até compensar
          payer: {
            email,
            first_name,
            last_name,
