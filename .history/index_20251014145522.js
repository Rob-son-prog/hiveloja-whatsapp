...
app.post('/mp/process-payment', async (req, res) => {
  try {
    ...
    // ✅ Um único Payment por requisição
...
if (method === 'card') {
  ...
  // IP (ajuda no antifraude)
  const clientIp =
    (req.headers['x-forwarded-for'] || '').split(',')[0].trim() ||
    req.socket?.remoteAddress ||
    undefined;

  // garante parcelas válidas
  ...
  const body = {
    ...
    // ⚠️ Não enviar payment_method_id / issuer_id para evitar diff_param_bins
  ...
  try {
    ...
    // se ainda vier erro do MP, devolve mensagem clara
    ...
    return res.status(400).json({ ok:false, error: err });
      ...
      // ===== BOLETO (aceita snake_case e camelCase no endereço) =====
      if (method === 'boleto') {
        ...
        // aceita ambos formatos: snake_case (zip_code) e camelCase (zipCode)
