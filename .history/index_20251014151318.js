...
/** Soma produto + bumps selecionados (flags = {id:true/false}) */
 ...
 * Formato leve do seu front: { orderId, productKey, method:'pix'|'boleto'|'card', bumps:{}, ... }
 */
app.post('/mp/process-payment', async (req, res) => {
  try {
    ...
    // === Caminho leve do checkout embutido ===
    if (req.body && req.body.method) {
      ...
      if (method === 'card') {
        ...
        try {
          ...
          // ✅ analytics: se aprovou na hora, já registra
          try {
            if ((p?.status || '').toLowerCase() === 'approved') {
              logEvent({ type: 'purchase', orderId, productKey, amount: Number(p?.transaction_amount ?? amount) });
            }
          } catch {}
      ...
      if (method === 'pix') {
        ...
        // ✅ se o PIX vier aprovado imediatamente (raro), registra
        try {
          if ((p?.status || '').toLowerCase() === 'approved') {
            logEvent({ type: 'purchase', orderId, productKey, amount: Number(p?.transaction_amount ?? amount) });
          }
      ...
      if (method === 'boleto') {
        ...
        // ✅ se por algum motivo o boleto vier aprovado já (muito raro), registra
        try {
          if ((p?.status || '').toLowerCase() === 'approved') {
            logEvent({ type: 'purchase', orderId, productKey, amount: Number(p?.transaction_amount ?? txAmount) });
          }
        } catch {}
    ...
    // ✅ se aprovado já aqui, registra
    try {
      if ((p?.status || '').toLowerCase() === 'approved') {
        logEvent({
          type: 'purchase',
          orderId: body?.metadata?.orderId || null,
          productKey: body?.metadata?.productKey || null,
          amount: Number(p?.transaction_amount ?? body?.transaction_amount ?? 0)
        });
      }
    } catch {}
...
app.post('/mp/webhook', async (req, res) => {
  ...
  try {
    ...
    if (!to) {
      ...
      // ainda assim registra a venda aprovada para o dashboard
      try {
        logEvent({ type: 'purchase', orderId, productKey: key, amount: Number(p?.transaction_amount || 0) });
      } catch {}
      return;
    }
    ...
    // ✅ registra analytics com o valor efetivamente pago
    try {
      logEvent({ type: 'purchase', orderId, productKey: key, amount: Number(p?.transaction_amount || 0) });
    } catch {}
