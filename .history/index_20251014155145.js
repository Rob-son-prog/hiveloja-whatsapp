...
if (req.body && req.body.method) {
  ...
  // base para webhook (sempre sem barra no final)
  const baseUrl = (process.env.APP_BASE_URL || '').replace(/\/+$/, '');
  ...
  if (method === 'card') {
    ...
    const body = {
      ...
      description: title,
      token: c.token,
      // intencionalmente NÃO envio payment_method_id/issuer_id para evitar diff_param_bins
      installments,
      external_reference: orderId,
      binary_mode: true,
      capture: true,
      notification_url: NOTIFY, // <- webhook para atualizar dashboard
      payer: {
  ...
  if (method === 'pix') {
    const body = {
      ...
      binary_mode: true,
      notification_url: NOTIFY, // <- webhook
      metadata: { orderId, productKey }
    };
  ...
  if (method === 'boleto') {
    ...
    const body = {
      ...
      external_reference: orderId,
      binary_mode: false, // boleto é pendente até compensar
      notification_url: NOTIFY, // <- webhook
      payer: {
        email,
        first_name,
...
app.post('/mp/webhook', async (req, res) => {
  ...
  try {
    ...
    // registra venda e receita
    try {
      const amount = Number(p.transaction_amount || p.transaction_details?.total_paid_amount || 0);
