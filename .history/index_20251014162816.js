...
app.post('/mp/webhook', async (req, res) => {
  ...
  try {
    ...
    const topic  = body.topic || query.topic || body.type || query.type; // 'payment'
    let id =
      body?.data?.id ||
      query?.data_id ||
      body?.resource  ||
    ...
    // se 'resource' vier como URL (/v1/payments/123456789), extrai só o número
    if (typeof id === 'string') {
    ...
    try {
      ...
      // tenta pegar o valor real do pagamento; se não, cai no preço do produto
      const amount =
