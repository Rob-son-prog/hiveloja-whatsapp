...
app.post('/mp/webhook', async (req, res) => {
  // responda rápido para não levar retry excessivo
  res.sendStatus(200);

  try {
    if (!process.env.MP_ACCESS_TOKEN || !mpClient) return;

    // Aceita ambos formatos: type/data.id (novo) e topic/resource/id (antigo)
    const topic =
      req.body?.topic ||
      req.query?.topic ||
      req.body?.type ||
    ...
    // coleta possíveis jeitos de vir o ID
    let id =
      req.body?.data?.id ||
      req.query?.data_id ||
      req.query?.id ||
    ...
    // às vezes vem em body.resource (string com número ou URL)
    if (!id && req.body?.resource) {
      const r = String(req.body.resource);
      const m = r.match(/(\d{6,})$/); // pega a cauda numérica
      if (m) id = m[1];
    }

    if (String(topic).toLowerCase() !== 'payment' || !id) {
