...
if (req.body && req.body.method) {
  ...
  // base para webhook (sem barra final)
  const baseUrl = (process.env.APP_BASE_URL || '').replace(/\/+$/, '');
  ...
  if (method === 'card') {
    ...
    const body = {
      ...
      description: title,
      token: c.token,
      // não enviar payment_method_id/issuer_id (evita diff_param_bins)
      installments,
  ...
  if (method === 'pix') {
    const body = {
      ...
      external_reference: orderId,        // <<< ADICIONADO
...
async function handleMpWebhook(req, res) {
  ...
  try {
    ...
    try {
      ...
      logEvent({ type: 'purchase', orderId: p.metadata?.orderId || null, productKey: key, amount });
    } catch (e) {
...
let ANALYTICS = readJsonSafe2(ANALYTICS_PATH, []); // [{ts, type, payment_id?, productKey?, orderId?, wa_id?, amount?}]

// helper: já registrou esse pagamento?
function alreadyLoggedPayment(paymentId){
  if (!paymentId) return false;
  return ANALYTICS.some(e => e.type === 'purchase' && e.payment_id === paymentId);
}
...
app.post('/campaigns/preview', (req, res) => {
  try {
    ...
    res.json({ ok: true, total_candidates: queue.length, sample });
  } catch (e) {
