...
try {
  ...
  if (alreadyLoggedPayment(paymentId, orderId)) {
    ...
    // 👇 NOVO: detecta método (pix / boleto / cartao / outro)
    ...
    // grava a compra com "method"
    // (use logEventSafe se você já tem; senão, pode usar logEvent)
...
app.get('/analytics/stats', (req, res) => {
  try {
    ...
    res.json({
      ...
      daily,
      payments // <<< ESSA LINHA QUE O FRONT USA
    });
  } catch (e) {
